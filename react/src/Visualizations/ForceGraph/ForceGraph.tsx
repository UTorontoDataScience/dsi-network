...
const ForceGraph: React.FC<ForceGraphProps> = ({
  ...
  const tree = useMemo(() => {
    return buildTree(rootModel, rootModelType, "root", links, selectedModels);
...
const registerTickHandler = <T extends ForceNode>(
  ...
  simulation.on("tick", () => {
    ...
    linkSelection
      .attr("x1", (d) => (d.source as ForceNodeSimulationWrapper<ForceNode>).x!)
      .attr("y1", (d) => (d.source as ForceNodeSimulationWrapper<ForceNode>).y!)
      .attr("x2", (d) => (d.target as ForceNodeSimulationWrapper<ForceNode>).x!)
      .attr(
        "y2",
